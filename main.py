...
# update the post 
@app.put("/api/posts/updatefull/{post_id}", response_model=schema.PostResponse)
def update_post(post_id:int, post_data:PostCreate, db:DBsession):
    result = db.execute(select(models.Post).where(models.Post.id == post_id))
    post = result.scalars().first()

    if not post:
        raise StarletteHTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "post not found")


    if post_data.user_id != post.auther_id:
    ...
    post__content = post_data.content
...
# partial update
@app.patch("/api/posts/partialupdate/{post_id}", response_model=schema.PostResponse)
def update_post_partial(post_id:int, post_data:schema.Postupdate, db:DBsession):
    post = db.get(models.Post, post_id)

    if not post:
        raise StarletteHTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "post not found")

    # convert fastapi pydantic into object by using model_dump(), exclude_unset = only provided fields
    update_data =  post_data.model_dump(exclude_unset=True)
    # dynamic set field 
    for field, value in update_data.items():
        # like post , post_title and its value ("title":"hi fastapi")
...
@app.delete("/api/post/delete/{post_id}", response_model = schema.PostResponse, status_code=status.HTTP_204_NO_CONTENT)
